...
The theorem [`fibContract_correct`](https://github.com/powdr-labs/yul-semantics/blob/main/YulSemantics/FibExample.lean#L184) says that for every initial state, running `fibContract` returns `Nat.fib n` (mod `2^256`), where `n` is the value read from calldata:
