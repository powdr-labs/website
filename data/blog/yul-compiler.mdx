...
Here is a complete Yul program written directly in Lean, using yul-semantics' embedded DSL `yul% { ... }` ([`FibExample.lean`](https://github.com/powdr-labs/yul-semantics/blob/main/YulSemantics/FibExample.lean#L24)). The program reads a number `n` from calldata and iteratively computes the nth Fibonacci number:
...
This is real Yul that happens to live inside Lean, which means we can state what the program is supposed to compute and prove it. The specification here is simply Lean's standard-library [`Nat.fib`](https://leanprover-community.github.io/mathlib4_docs/Mathlib/Data/Nat/Fib/Basic.html), so there is nothing bespoke to trust on the spec side.

The theorem [`fibContract_correct`](https://github.com/powdr-labs/yul-semantics/blob/main/YulSemantics/FibExample.lean#L168) says that for every initial state, running `fibContract` halts, writes `Nat.fib n` (mod `2^256`) to memory slot `0`, and returns that 32-byte word, where `n` is the value read from calldata:
...
theorem fibContract_correct (st0 : EvmState) :
    ∃ st, Run evm fibContract st0 [] st .halt ∧
      ...
      st.halted = some (.ret, readBytes st.memory 0 32)
```
...
The main theorem is [`compile_correct`](https://github.com/powdr-labs/yul-compiler/blob/main/YulEvmCompiler/Correctness.lean#L1580), which says the compiler does not lie about what your program does:
